...
Phase 2 (async execution): queue-backed worker (BullMQ/Redis or equivalent) performs reconciliation, subscription/order/entitlement updates, audit logs, retries with backoff, and replay endpoint `POST /api/webhooks/razorpay?replay=:id (folded into the main handler 2026-08-19, no longer its own function)` (admin-only, idempotent requeue).
