...
export async function processWebhookEvent(eventId) {
  ...
  try {
    ...
    // Process invoice generation on successful payment capture
    if (event.event_type === "payment.captured") {
      console.log(`[Billing] Webhook payment.captured received. Creating invoice for Event ID: ${eventId}`);
      await createInvoiceForPayment(payload);
    }
