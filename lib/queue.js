// Node's crypto, explicitly. Without this import `crypto` resolves to the
// global Web Crypto object, which has getRandomValues but no randomBytes —
// so the manual-review fallback below threw "crypto.randomBytes is not a
// function" at exactly the moment it was trying to record a failed payment.
import crypto from "crypto";
import { getDB } from "./db.js";
import { logEvent } from "./logger.js";
import { createInvoiceForPayment } from "./invoiceEngine.js";
import { createOrUpdatePassport, getProductMetadata, revokePassportEntitlements } from "./passportEngine.js";
import { sendDeliveryEmail, sendRefundEmail } from "./emailService.js";


const queue = [];
let draining = false;

export function enqueue(task) {
  queue.push({ task, retries: 0 });
  void processQueue();
}

export async function processQueue() {
  if (draining) return;
  draining = true;

  while (queue.length > 0) {
    const item = queue[0];

    try {
      await item.task();
      queue.shift();
    } catch (err) {
      item.retries += 1;
      if (item.retries > 3) {
        console.error("Failed permanently:", err);
        queue.shift();
      }
    }
  }

  draining = false;
}

export async function processWebhookEvent(eventId) {
  const db = await getDB();
  const event = await db.get("SELECT * FROM webhook_events WHERE id = ?", [eventId]);
  if (!event || event.processing_state === "PROCESSED") return;
  try {
    await db.run("UPDATE webhook_events SET processing_state = 'PROCESSING', processing_attempts = processing_attempts + 1 WHERE id = ?", [eventId]);
    const payload = JSON.parse(event.payload_json || "{}");
    await logEvent("razorpay_webhook_received", { eventId, eventType: event.event_type, payloadId: payload?.id || null });

    const eventType = event.event_type;
    let paymentEntity = null;
    let orderEntity = null;
    let paymentLinkEntity = null;
    let subscriptionEntity = null;

    if (payload?.payload) {
      paymentEntity = payload.payload.payment?.entity;
      orderEntity = payload.payload.order?.entity;
      paymentLinkEntity = payload.payload.payment_link?.entity;
      subscriptionEntity = payload.payload.subscription?.entity;
    }

    if (eventType === 'payment.captured' || eventType === 'order.paid' || eventType === 'payment_link.paid' || eventType === 'subscription.charged') {
      const paymentId = paymentEntity?.id;
      const subscriptionId = subscriptionEntity?.id || paymentEntity?.subscription_id || null;
      const orderId = subscriptionId || paymentEntity?.order_id || orderEntity?.id || paymentLinkEntity?.id || paymentId;
      const amount = paymentEntity?.amount || orderEntity?.amount || paymentLinkEntity?.amount || 0;
      const currency = paymentEntity?.currency || orderEntity?.currency || paymentLinkEntity?.currency || 'INR';

      const notes = paymentEntity?.notes || orderEntity?.notes || paymentLinkEntity?.notes || {};
      const email = paymentEntity?.email || notes.customer_email || notes.email || '';
      const userId = notes.user_id || notes.customer_id || '';
      const productId = notes.product_id || '';
      const articleSlug = notes.article_slug || '';
      const newsletterSlug = notes.newsletter_slug || '';

      const expiresAt = new Date();
      expiresAt.setDate(expiresAt.getDate() + 30);
      const expiresAtStr = expiresAt.toISOString();

      if (orderId) {
        const existingOrder = await db.get("SELECT * FROM orders WHERE id = ?", [orderId]);
        if (existingOrder) {
          await db.run(
            `UPDATE orders 
             SET status = 'paid', payment_id = ?, verified = 1, expires_at = ?
             WHERE id = ?`,
            [paymentId, expiresAtStr, orderId]
          );
          await logEvent("payment_processed_webhook", {
            razorpay_order_id: orderId,
            razorpay_payment_id: paymentId,
            email: existingOrder.email || email,
            product_id: productId,
            status: "updated"
          });
        } else {
          await db.run(
            `INSERT INTO orders (id, amount, currency, status, payment_id, verified, email, user_id, article_slug, newsletter_slug, expires_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [orderId, amount, currency, 'paid', paymentId, 1, email, userId, articleSlug, newsletterSlug, expiresAtStr]
          );
          await logEvent("payment_processed_webhook", {
            razorpay_order_id: orderId,
            razorpay_payment_id: paymentId,
            email: email,
            product_id: productId,
            status: "inserted"
          });
        }

        await db.run(
          `INSERT INTO events (type, payload) VALUES (?, ?)`,
          ['payment_completed', JSON.stringify({ 
            razorpay_order_id: subscriptionId ? null : orderId, 
            razorpay_subscription_id: subscriptionId,
            razorpay_payment_id: paymentId, 
            product_id: productId, 
            user_id: userId,
            email: email,
            article_slug: articleSlug,
            newsletter_slug: newsletterSlug,
            source: 'webhook'
          })]
        );

        // 1. Generate the Invoice PDF & DB records
        let invoice = null;
        let failureReason = null;
        try {
          invoice = await createInvoiceForPayment(payload);
          console.log(`[Billing] Generated Invoice: ${invoice.invoice_number}`);
        } catch (invErr) {
          console.error("[Billing] Failed to generate invoice in queue worker:", invErr);
          failureReason = "Invoice Generation Failed";
        }

        // 2. Generate/Update the Customer Passport
        let passport = null;
        try {
          passport = await createOrUpdatePassport({
            email: email,
            name: paymentEntity?.notes?.customer_name || orderEntity?.notes?.customer_name || paymentLinkEntity?.notes?.customer_name || "",
            orderId: subscriptionId ? null : orderId,
            razorpaySubscriptionId: subscriptionId,
            productId: productId,
            razorpayCustomerId: paymentEntity?.customer_id || "",
            country: notes.country || null,
            timezone: notes.timezone || null,
            locale: notes.locale || null
          });
          console.log(`[Passport] Assigned Passport ID: ${passport.passport_id}`);
        } catch (passErr) {
          console.error("[Passport] Failed to generate passport in queue worker:", passErr);
          failureReason = failureReason ? `${failureReason} | Passport Generation Failed` : "Passport Generation Failed";
        }

        // 3. Send Delivery & Onboarding Email
        let emailSent = false;
        if (passport && invoice) {
          try {
            const meta = getProductMetadata(productId);
            await sendDeliveryEmail({
              email: email,
              customerName: passport.customer_name,
              passportId: passport.passport_id,
              productName: meta.productName,
              downloadLink: meta.downloadLink,
              invoicePath: invoice.pdf_path,
              // Per-product copy. onboardingSteps has existed in the product
              // metadata since the beginning and was never passed through, so
              // every purchase received the same generic "Download Asset" mail
              // regardless of what was bought.
              deliveryNote: meta.deliveryNote,
              ctaLabel: meta.ctaLabel,
              onboardingSteps: meta.onboardingSteps
            });
            console.log(`[Email] Onboarding and delivery email dispatched to ${email}`);
            emailSent = true;
          } catch (emailErr) {
            console.error("[Email] Failed to send delivery email:", emailErr);
            failureReason = failureReason ? `${failureReason} | Email Delivery Failed` : "Email Delivery Failed";
          }
        } else {
          failureReason = failureReason || "Delivery aborted due to previous failure";
        }

        // 4. Post Payment Validation / Manual Review Queue
        if (failureReason || !emailSent || !passport || !invoice) {
           const finalReason = failureReason || "Unknown delivery failure";
           await db.run(
              `INSERT INTO manual_review_queue (id, order_id, payment_id, email, product_id, failure_reason)
               VALUES (?, ?, ?, ?, ?, ?)`,
              [crypto.randomBytes(8).toString('hex'), orderId, paymentId, email, productId, finalReason]
           );
           console.error(`[Manual Review] Order ${orderId} queued for review. Reason: ${finalReason}`);
        }
      }
    } else if (eventType === 'payment.refunded') {
      const paymentId = paymentEntity?.id || payload?.payload?.refund?.entity?.payment_id;
      const refundId = payload?.payload?.refund?.entity?.id;
      const order = await db.get("SELECT * FROM orders WHERE payment_id = ?", [paymentId]);
      if (order) {
        await db.run("UPDATE orders SET status = 'refunded' WHERE id = ?", [order.id]);
        let productId = "test_product";
        const notes = paymentEntity?.notes || orderEntity?.notes || {};
        if (notes.product_id) {
          productId = notes.product_id;
        } else {
          const payloadNotes = payload?.payload?.payment?.entity?.notes || {};
          if (payloadNotes.product_id) {
            productId = payloadNotes.product_id;
          }
        }
        const passport = await revokePassportEntitlements({
          email: order.email,
          productId: productId
        });
        if (passport) {
          try {
            await sendRefundEmail({
              email: order.email,
              customerName: passport.customer_name,
              productName: getProductMetadata(productId).productName
            });
            console.log(`[Refund] Revocation email sent to ${order.email}`);
          } catch (emailErr) {
            console.error("[Refund] Failed to send revocation email:", emailErr);
          }
        }
        await db.run(
          `INSERT INTO events (type, payload) VALUES (?, ?)`,
          ['refund_completed', JSON.stringify({ 
            payment_id: paymentId, 
            refund_id: refundId,
            email: order.email,
            product_id: productId
          })]
        );
      } else {
        console.warn(`[Refund] No order found matching payment_id: ${paymentId}`);
      }
    }

    await db.run("UPDATE webhook_events SET processing_state = 'PROCESSED', processed_at = CURRENT_TIMESTAMP, last_error = NULL WHERE id = ?", [eventId]);
  } catch (error) {
    await db.run("UPDATE webhook_events SET processing_state = 'FAILED', last_error = ? WHERE id = ?", [error?.message || "unknown", eventId]);
    console.error(JSON.stringify({ provider: "razorpay", type: "worker_error", eventId, error: error?.message || "unknown", ts: new Date().toISOString() }));
    throw error;
  }
}
