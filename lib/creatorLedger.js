...
export async function ensureTables() {
  ...
  await db.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_creator_booking_order ON creator_bookings(order_id);
  `);
  tablesCreated = true;
}
...
export async function getOrderByRazorpayOrderId(razorpayOrderId) {
  ...
  const row = await db.get(
    'SELECT order_id FROM creator_orders WHERE razorpay_order_id = ?',
    [razorpayOrderId]
  );
...
export async function updateOrderStatus(orderId, status, extra) {
  ...
  const current = await db.get('SELECT status FROM creator_orders WHERE order_id = ?', [orderId]);
  if (current) {
...
export async function createBooking(record) {
  ...
  const db = await getDB();
  await db.run(
    `INSERT OR IGNORE INTO creator_bookings (booking_id, order_id, creator_slug, offer_id, offer_title,
