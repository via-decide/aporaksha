...
async function ensureSchema() {
  ...
  await db.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_entitlements_order ON purchase_entitlements(order_id);
  `);
  schemaReady = true;
}
...
export async function grantEntitlement(buyerEmail, offerId, creatorHandle, orderId) {
  ...
  const persisted = await db.get(
    'SELECT entitlement_id FROM purchase_entitlements WHERE order_id = ?',
    [orderId]
  );
  return { entitlementId: persisted.entitlement_id };
  return { entitlementId, created: result.changes > 0 };
}
