...
async function ensureSchema() {
  ...
  await db.exec(`
    ...
    -- Older membership webhooks used the subscription id for every charge, so
    -- databases may already contain several rows for the key now used to make
    -- webhook retries idempotent. Keep the earliest such row before adding the
    -- constraint; otherwise schema initialization fails on those databases.
    DELETE FROM financial_ledger AS duplicate
      WHERE duplicate.entry_type = 'membership'
        AND duplicate.provider IS NOT NULL
        AND duplicate.provider_txn_id IS NOT NULL
        AND EXISTS (
          SELECT 1
          FROM financial_ledger AS keeper
          WHERE keeper.entry_type = 'membership'
            AND keeper.provider = duplicate.provider
            AND keeper.provider_txn_id = duplicate.provider_txn_id
            AND (
              keeper.created_at < duplicate.created_at
              OR (keeper.created_at = duplicate.created_at AND keeper.entry_id < duplicate.entry_id)
            )
        );
