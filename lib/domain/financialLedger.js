 ...
 *   refund           reversal of a sale (buyer-side)
 *   fee_reversal     reversal of platform fee (on refund)
 *   creator_reversal reversal of creator payout (on refund)
...
export async function recordSale(params) {
  ...
  await db.run(
    `INSERT INTO financial_ledger (entry_id, entry_type, order_id, creator_handle, buyer_email, amount_minor, currency, description, provider, provider_txn_id)
  ...
  await db.run(
    ...
    [feeId, orderId, creatorHandle, fee, currency, saleId]
  );

  await db.run(
    `INSERT INTO financial_ledger (entry_id, entry_type, order_id, creator_handle, amount_minor, currency, description, reference_entry_id)
     VALUES (?, 'creator_payout', ?, ?, ?, ?, 'Creator payout (99%)', ?)`,
    [payoutId, orderId, creatorHandle, payout, currency, saleId]
  );

  return { saleId, feeId, payoutId, fee, payout };
}
...
export async function recordRefund(params) {
  ...
  const creatorReversalId = crypto.randomUUID();
  ...
  await db.run(
    `INSERT INTO financial_ledger (entry_id, entry_type, order_id, creator_handle, amount_minor, currency, description, reference_entry_id)
     VALUES (?, 'creator_reversal', ?, ?, ?, ?, 'Creator payout reversal on refund', ?)`,
    [creatorReversalId, orderId, creatorHandle, -payout, currency, originalSaleId]
  );

  return { refundId, feeReversalId, creatorReversalId };
}
...
export async function recordMembership(params) {
  ...
  await db.run(
    `INSERT INTO financial_ledger (entry_id, entry_type, buyer_email, amount_minor, currency, description, provider, provider_txn_id)
...
export async function getCreatorBalance(creatorHandle) {
  ...
  const row = await db.get(
    `SELECT COALESCE(SUM(amount_minor), 0) as balance
     ...
     WHERE creator_handle = ? AND entry_type IN ('creator_payout', 'creator_reversal')`,
    [creatorHandle]
  );
