 ...
 */
function getHtmlTemplate({ customerName, passportId, productName, downloadLink, deliveryNote, ctaLabel, onboardingSteps }) {
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="wrapper">
    <div class="container">
      ...
      <div class="content">
        ...
        ${deliveryNote ? `<p style="background:#0f172a;border-left:3px solid #ff671f;padding:14px 16px;margin:18px 0;color:#cbd5e1;font-size:14px;line-height:1.6;">${deliveryNote}</p>` : ''}
        ...
        <div class="product-meta">
          <table>
            ...
            <tr>
              ...
              <td class="meta-val"><a href="${downloadLink}" style="color:#ff671f; text-decoration:underline;" target="_blank">${ctaLabel || "Download Asset"}</a></td>
            </tr>
          </table>
        </div>

        ${Array.isArray(onboardingSteps) && onboardingSteps.length ? `
        <p style="margin:22px 0 8px;color:#94a3b8;font-size:13px;letter-spacing:1px;text-transform:uppercase;">What happens next</p>
        <ol style="color:#cbd5e1;font-size:14px;line-height:1.9;padding-left:20px;margin:0 0 6px;">
          ${onboardingSteps.map(x => `<li>${x}</li>`).join('')}
        </ol>` : ''}
 ...
 */
export async function sendDeliveryEmail({ email, customerName, passportId, productName, downloadLink, invoicePath, deliveryNote, ctaLabel, onboardingSteps }) {
  ensureEmailDir();

  const subject = `[Aporaksha] Passport ${passportId} Activated - ${productName}`;
  const htmlContent = getHtmlTemplate({ customerName, passportId, productName, downloadLink, deliveryNote, ctaLabel, onboardingSteps });
 ...
 */
export async function sendRefundEmail({ email, customerName, productName }) {
  ensureEmailDir();

  const subject = `[Aporaksha] Refund Processed - ${productName}`;
  const htmlContent = `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    body { background-color: #0b0c10; color: #f5f5f5; font-family: sans-serif; padding: 40px 20px; }
    .container { max-width: 600px; margin: 0 auto; background-color: #121826; border: 1px solid #1e293b; border-radius: 12px; padding: 40px; }
    h1 { color: #ff671f; }
    p { color: #acacac; line-height: 1.6; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <p>Hello ${customerName},</p>
    <p>Your refund request for <strong>${productName}</strong> has been successfully processed.</p>
    <p>Please note that your access entitlements have been revoked and your download link deactivated. The refunded amount will reflect in your original payment method in 5-7 business days.</p>
    <p>Best regards,<br>Aporaksha Core Team</p>
  </div>
</body>
</html>
  `;
  ...
  if (host && user && pass) {
    try {
      ...
      console.log(`[Email] Refund email sent successfully via SMTP: ${info.messageId}`);
      sentToSmtp = true;
    } catch (smtpErr) {
      console.error("[Email] SMTP sending failed for refund, falling back to mock file:", smtpErr);
    }
  }

  const mockFileName = `refund-${email}-${Date.now()}.html`;
  const mockPath = path.join(EMAIL_OUT_DIR, mockFileName);
  fs.writeFileSync(mockPath, htmlContent, "utf8");
  console.log(`[Email] Local audit copy generated: ${mockPath}`);
 ...
 * Ensures we can deliver emails before taking payment.
 */
export async function checkHealth() {
  ...
  if (!host || !user || !pass) {
    // If not in development mode, lack of SMTP means we can't deliver.
    if (process.env.NODE_ENV === 'development' || process.env.ALLOW_LOCAL_EMAIL !== 'false') {
      return true;
    }
    console.error("[SMTP Health] Credentials missing");
    return false;
  }
  ...
  try {
    ...
    console.error("[SMTP Health] Verification failed:", error);
    return false;
  }
}
