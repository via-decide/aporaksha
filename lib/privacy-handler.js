...
import { authenticatedSubject } from "./privacy-auth.js";
import { accessReport, audit, consent, createRequest, initPrivacy, now, opaque, rejectBrowserIdentity, uuid } from "./privacy.js";
import { getDB, withTransaction } from "./db.js";

const json = (res, status, body) => res.status(status).json(body);
const pathOf = req => {
  if (Array.isArray(req.query?.path)) return req.query.path.filter(Boolean);
  if (req.query?.path) return String(req.query.path).split("/").filter(Boolean);
  if (req.query?.privacyPath) return String(req.query.privacyPath).split("/").filter(Boolean);
  const pathname = String(req.url || "").split("?")[0];
  const marker = "/api/privacy/";
  const index = pathname.indexOf(marker);
  return index === -1 ? [] : pathname.slice(index + marker.length).split("/").filter(Boolean);
};
function safeQuery(req) {
  const extra = Object.keys(req.query || {}).filter(k => !["path", "privacyPath"].includes(k));
...
export default async function handler(req, res) {
  try {
    ...
    if (resource === "erasure" && req.method === "POST") {
      ...
      const requestId = uuid(), evidenceId = uuid(), hash = opaque(subject), timestamp = now();
      const retained = await db.get("SELECT COUNT(*) AS count FROM orders WHERE user_id=?", [subject]);
      const retainedCategories = ["minimal_account_tombstone", ...(retained?.count ? ["transaction_records"] : [])];
      const retainedReasons = ["ERASURE_SUPPRESSION_INTEGRITY", ...(retained?.count ? ["STATUTORY_TRANSACTION_RETENTION_REVIEW"] : [])];
      await withTransaction(db, async tx => {
        await tx.run("INSERT INTO privacy_requests(request_id,principal_identity_id,request_type,status,reason_code,requested_at,identity_verified_at) VALUES(?,?,?,?,?,?,?)", [requestId, hash, "ERASURE", "IDENTITY_VERIFIED", null, timestamp, timestamp]);
        await tx.run("INSERT INTO privacy_request_events(event_id,request_id,event_type) VALUES(?,?,?)", [opaque(body.reauthenticationNonce), requestId, "REAUTH_PROOF_CONSUMED"]);
        await tx.run("INSERT INTO privacy_audit_events(event_id,principal_reference,event_type,object_reference) VALUES(?,?,?,?)", [uuid(), hash, "PRIVACY_ERASURE_REQUESTED", requestId]);
        await tx.run("DELETE FROM auth_sessions WHERE principal_identity_id=?", [subject]);
        await tx.run("DELETE FROM privacy_corrections WHERE request_id IN (SELECT request_id FROM privacy_requests WHERE principal_identity_id=?)", [subject]);
        await tx.run("DELETE FROM privacy_grievances WHERE principal_identity_id=?", [subject]);
        await tx.run("DELETE FROM privacy_nominations WHERE principal_identity_id=?", [subject]);
        await tx.run("DELETE FROM consent_records WHERE principal_identity_id=?", [subject]);
        await tx.run("DELETE FROM privacy_notice_deliveries WHERE principal_identity_id=?", [subject]);
        await tx.run("UPDATE privacy_requests SET principal_identity_id=? WHERE principal_identity_id=?", [hash, subject]);
        await tx.run("UPDATE passports SET customer_name=NULL,email=NULL,password_hash=NULL,nfc_chip_id=NULL,country=NULL,timezone=NULL,locale=NULL,activation_status='ERASED' WHERE passport_id=?", [subject]);
        await tx.run("INSERT INTO privacy_erasure_evidence(evidence_id,request_id,opaque_subject_hash,categories_deleted,categories_anonymized,categories_retained,retention_reason_codes,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)", [evidenceId, requestId, hash, JSON.stringify(["identity_credentials","sessions","correction_values","grievance_content","nomination_contact","consent_records","notice_deliveries"]), JSON.stringify(["privacy_request_subject_links"]), JSON.stringify(retainedCategories), JSON.stringify(retainedReasons), "COMPLETED_WITH_RETENTION", timestamp]);
        await tx.run("INSERT OR REPLACE INTO erasure_suppression(opaque_subject_hash,erased_at,reason_code) VALUES(?,?,?)", [hash, timestamp, "PRINCIPAL_ERASURE"]);
        await tx.run("UPDATE privacy_requests SET status='COMPLETED_WITH_RETENTION',completed_at=?,completion_evidence_reference=? WHERE request_id=?", [timestamp, evidenceId, requestId]);
        await tx.run("INSERT INTO privacy_audit_events(event_id,principal_reference,event_type,object_reference) VALUES(?,?,?,?)", [uuid(), hash, "PRIVACY_ERASURE_COMPLETED", requestId]);
      });
      return json(res, 202, { requestId, status: "COMPLETED_WITH_RETENTION", retainedReasonCodes: retainedReasons });
    }
