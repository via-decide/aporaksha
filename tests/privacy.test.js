...
test("refresh preserves strong-authentication time and permits only one concurrent rotation", async () => {
  const [loginReq,loginRes] = mock({action:"signup",email:"refresh@test.invalid",password:"StrongPassword!2",adultConfirmed:true});
  await authHandler(loginReq,loginRes); assert.equal(loginRes.code,201);
